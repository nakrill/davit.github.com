/* ------------------------------------------------------------------------
	...
	Based on Version: 3.0 - Adopted to Cincopa
------------------------------------------------------------------------- */

(function(zeQuery) {
	zeQuery.prettyPhoto = { version: '3.0' };

	zeQuery.fn.prettyPhoto = function(pp_settings) {
		var $=zeQuery;
		...
		*/
		$.prettyPhoto.open = function() {
			if(arguments.length) { // Means it's an API call, need to manually get the settings and set the variables
				settings = pp_settings;
				_buildOverlay(this); // Build the overlay {this} being the caller
			...
			callback_cookie = grab_param('callback_cookie', pp_images[set_position]);
		...
		*/
		function _fitToViewport(width, height) {
			...
			if (settings.fit_default_dim && _getFileType(pp_images[set_position]) == 'image') {
		...
		*/
		...
		function _buildOverlay(caller) {
			...
			pp_images = (isSet) ? $.map(matchedObjects, function(n, i) { if ($(n).attr('rel').indexOf(theRel) != -1) return $(n).attr('href'); }) : $.makeArray($(caller).attr('href'));
			pp_titles = (isSet) ? $.map(matchedObjects, function(n, i) { if ($(n).attr('rel').indexOf(theRel) != -1) return ($(n).find('img').attr('alt')) ? $(n).find('img').attr('alt') : ""; }) : $.makeArray($(caller).find('img').attr('alt'));
			pp_descriptions = (isSet) ? $.map(matchedObjects, function(n, i) { if ($(n).attr('rel').indexOf(theRel) != -1) return ($(n).attr('title')) ? $(n).attr('title') : ""; }) : $.makeArray($(caller).attr('title'));
		...
		return this.unbind('click').click($.prettyPhoto.initialize); // Return the $ object for chaining. The unbind method is used to avoid click conflict when the plugin is called more than once
	};
